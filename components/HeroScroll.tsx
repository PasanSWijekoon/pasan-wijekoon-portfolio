...
import { ChevronDown, Github, Linkedin, Mail, Sparkles, Code2, Cpu, GitBranch } from "lucide-react";

const frameCount = 192;
const frameUrls = Array.from({ length: frameCount }, (_, i) =>
  `/animations/${(i + 1).toString().padStart(5, "0")}.png`
);
...
export default function HeroScroll() {
  ...
  const softSkillsOpacity = useTransform(scrollYProgress, [0.15, 0.25, 0.35], [0, 1, 0]);
  
  // 2. Core Practices (OOP, MVC, Optimization) - 35% to 50%
  const practicesOpacity = useTransform(scrollYProgress, [0.35, 0.42, 0.50], [0, 1, 0]);

  // 3. Methodologies & DevOps (Agile, Microservices, CI/CD) - 50% to 65%
  const methodsOpacity = useTransform(scrollYProgress, [0.50, 0.57, 0.65], [0, 1, 0]);
  ...
  useEffect(() => {
    ...
    resizeCanvas();
    const unsubscribe = currentIndex.on("change", (latest) => requestAnimationFrame(() => render(latest)));
    render(currentIndex.get());
    return () => {
      unsubscribe();
  ...
  const marqueeText = [
    "I’m a programmer and designer who bridges visual design and engineering — I design intuitive interfaces and implement them with clean, maintainable code.",
    ...
    <div ref={containerRef} className="h-[500vh] relative bg-[#02040a]">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center">
        ...
        >
            <div className="flex-1 flex flex-col items-center justify-center w-full max-w-7xl px-6 md:px-12 text-center space-y-8">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/20 bg-cyan-500/5 backdrop-blur-sm">
                    ...
                    <span className="text-xs font-mono text-cyan-400 tracking-wider">OPEN FOR WORK 2026</span>
                </div>
                ...
                <p className="max-w-xl mx-auto text-slate-400 text-lg font-light leading-relaxed">
                    Building robust <span className="text-cyan-400">backend systems</span> and scalable <span className="text-cyan-400">cloud infrastructure</span> with engineering precision.
                </p>

                {/* Social Row */}
                <div className="flex items-center justify-center gap-6 pt-4 pointer-events-auto">
                    <a href="https://github.com/PasanSWijekoon" target="_blank" rel="noopener noreferrer" className="p-3 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-cyan-400 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all duration-300">
                        <Github size={20} />
                    </a>
                    <a href="https://www.linkedin.com/in/pasan-s-wijekoon/" target="_blank" rel="noopener noreferrer" className="p-3 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-cyan-400 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all duration-300">
                        <Linkedin size={20} />
                    </a>
                    <a href="mailto:wijekoonpasan055@gmail.com" className="p-3 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-cyan-400 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all duration-300">
                        <Mail size={20} />
                    </a>
                </div>
            </div>
        ...
        <div className="absolute inset-0 pointer-events-none z-30 mix-blend-screen w-full h-full max-w-7xl mx-auto px-6 md:px-12">
            ...
            <motion.div style={{ opacity: practicesOpacity }} className="absolute inset-0">
                ...
                <div className="absolute top-[25%] left-0 md:-left-[8%] group">
                    <div className="relative glass-card px-8 py-5 rounded-2xl border-glow bg-cyan-950/30 transform rotate-1">
                        <div className="text-cyan-400 font-bold text-2xl md:text-4xl">OOP Principles</div>
                ...
                <div className="absolute top-[50%] right-0 md:-right-[8%] group">
                    <div className="relative glass-card px-8 py-5 rounded-2xl border-glow bg-white/5 transform -rotate-1">
                        <div className="text-white font-bold text-2xl md:text-4xl">MVC Architecture</div>
                ...
                <div className="absolute bottom-[25%] left-0 md:-left-[8%] group">
                    <div className="relative glass-card px-8 py-5 rounded-2xl border-glow bg-blue-950/30 flex items-center gap-3 transform -rotate-2">
                        ...
                        <div className="text-blue-400 font-bold text-2xl md:text-4xl">Code Optimization</div>
                        {/* Longer Connector Line Right */}
            ...
            <motion.div style={{ opacity: methodsOpacity }} className="absolute inset-0">
                {/* Agile - Top Left */}
            ...
            {/* 4. FINAL CTA */}
            <motion.div style={{ opacity: ctaOpacity }} className="absolute inset-0 flex flex-col items-center justify-center">
                <h2 className="text-5xl md:text-8xl font-bold text-white tracking-tight mb-8 text-center">
                    Let's Build <br/> The Future
                </h2>
                <div className="pointer-events-auto">
                    <a href="#contact" className="inline-flex items-center justify-center px-8 py-4 bg-cyan-500 text-black rounded-full font-bold hover:bg-cyan-400 hover:shadow-[0_0_30px_rgba(0,240,255,0.4)] transition-all duration-300">
                        Start a Project
                    </a>
                </div>
            </motion.div>
